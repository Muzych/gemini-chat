...
export default function Message({ content, error, role = "user", thinking = false }: MessageProps) {
    ...
    useEffect(() => {
        ...
        return () => {
            theMessageWrap.removeEventListener('trasitionend', handleTransitionEnd);
        }
    }, []);

    return (
        <div
            className={clsx(`message message-${role} w-full flex small-mobile:max-sm:text-sm transiton-height duration-300`,
        ...
        >
            <div className={clsx('message-inner space-x-4 max-w-[480px] rounded-3xl p-4 transition-[opacity, transform] duration-300 delay-250 relative text-sm md:text-base',
